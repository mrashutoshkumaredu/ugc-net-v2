...
let pendingExamConfig = null; // Stores mode, count, and duration before launch
...
function showLandingPage() {
  if (!isSubmitted && questions.length > 0 && confirm("Leaving now will abandon your active test. Return to dashboard?")) {
...
function prepareTest(mode) {
  if (!currentUser) {
    alert("Please log in or register to begin your practice test.");
    toggleAuthModal();
    return;
  }
  ...
  if (mode === 'FULL') {
    ...
    pendingExamConfig = {
      mode: selectedTheme,
      questions: filtered,
      duration: requestedCount * 60 // 1 min per question in seconds
    };
  }

  // Show Pre-Exam Instruction Modal
...
async function submitTest(isAuto = false) {
  if (!isAuto && !isSubmitted && !confirm("Are you sure you want to submit your test?")) return;
  ...
  alert(`Test Submitted Successfully!\nScore: ${score} / ${questions.length} (${percentage}%)\nTime Taken: ${Math.floor(timeTaken / 60)} mins ${timeTaken % 60} secs`);
